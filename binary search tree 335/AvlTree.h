...
class AvlTree
{
  ...
  private:
    ...
    */
    double Internal_PathLength(AvlNode* & t)
    {
        if(t==nullptr)
            return 0;
        else
    ...
    */
    double NumberOfNodes(AvlNode * & t)
    {
        if(t==nullptr)
            return 0;
        else
     ...
     * t is the node that roots the subtree.
     * Set the new root of the subtree.
     * Count is the number recursion calls. return if negative or positive.
     */
     ...
     */
    int contains( const Comparable & x, AvlNode *t, int count) 
    {
        if( t == nullptr )
            return (count*-1);
        else if( x < t->element )
